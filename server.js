  ...
  db.query("SELECT * from employee", async (err, results) => {
    ...
    db.query("SELECT * FROM role", async (err, roleresult) => {
      ...
      db.query(
        ...
        async (err, mangresults) => {
          try {
            const newManager = await inquirer
              .prompt([
                {
                  type: "list",
              ...
              .then(function (answers) {
                ...
                db.query(
                  ...
                  function (err, newEmpResults) {
                    if (err) throw err;
                    console.log(
                      "A New Employee has been added to your Database!",
                    );
                    start();
                  },
                );
...
updateEmployee = () => {
  db.query(
    "SELECT id, role_id, CONCAT (first_name,' ', last_name) AS name FROM employee",
    async (err, res) => {
      try {
        const editedEmployee = await inquirer
          .prompt([
            {
              type: "list",
              name: "updateEmp",
